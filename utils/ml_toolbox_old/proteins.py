...
class Sample:
    ...
    Set of properties and methods associated with a single sample. Data can be extracted as follows:
        ...
        vox = [d.header_map.voxs for d in data]
    ...
    * Subcubes are treated as members of the sample to prevent them from
    ...
    class Header:
        def __init__(self, o, s, c):
            ...
            self.samp = s
            self.cell = c
            self.voxs = (c.x / s[0], c.y / s[1], c.z / s[2])

    def __init__(self, res, xpath, ypath=None, wpath=None):

        self.res = res
        self.id = xpath.split('/')[-1].split('_')[0]
    ...
    def save_map(self, path=None, map_rec=False):
        if map_rec:
            ...
            map = self.map
        if path is None:
            path = '/'.join(self.base_path[:-1]) + 'maps/' + self.base_path[-1]  # self.path_map[:-9] + 'maps' + self.path_map[-9:]
        gpath = path.split('/')
        if len(gpath) > 1:
    ...
    def save_lab(self, path=None):
        ...
        if path is None:
            path = '/'.join(self.base_path[:-1]) + 'labs/' + self.base_path[-1]  # self.path_map[:-9] + 'labs' + self.path_map[-9:]
        gpath = path.split('/')
        if len(gpath) > 1:
            if not os.path.exists('/'.join(gpath[:-1])):
                os.mkdir('/'.join(gpath[:-1]))
    ...
    def decompose(self, cshape=64, margin=8, norm=True, crop=True, step=None,
        ...
        """
        # TODO apply normalisation (+ move normalise function from multi to utils)
                ...
                for k in range(0, dat.shape[2], self.step)[:-1]:
                    ...
                    if background_limit is not None:
                        ...
                        if background_label is not None:
                            ...
                            # print("Threshold: max", np.max(window_array[:,:,window_array.shape[-1]//2]), "min", np.min(window_array[:,:,window_array.shape[-1]//2]), "<=", threshold)
                            # print("Threshold: max", np.max(window_array), "min", np.min(window_array), "<=", threshold)
                            # plt.imshow(window_array[:,:,window_array.shape[-1]//2], vmin=0, vmax=1)
                            # plt.figure()
                            # win = np.copy(window_array)
                            # win[window_array <= threshold] = 0
                            # plt.imshow(win[:,:,win.shape[-1]//2], vmin=0, vmax=1)
                            # plt.show()
                            if np.sum(window_array <= threshold)/float(window_array.size) \
                                    > background_limit: continue
                            self.tile_indices.append((i,j,k))   # save selected tile indices
                        #print("Background:", np.sum(window_array <= threshold) / float(window_array.size), ">",
                        #      background_limit)
                        else:
                    ...
                    # print("Tile:", self.tiles[-1].shape)
                    # plt.imshow(self.tiles[-1][:,:,cshape//2])
                    # plt.show()
                    if self.lab is not None:
        ...
        if len(self.tiles) == 0:
            self.crop_tile = True
            z_start = max(0,int(dat.shape[0]/2 - cshape/2))
            y_start = max(0,int(dat.shape[1]/2 - cshape/2))
            x_start = max(0,int(dat.shape[2]/2 - cshape/2))
            sh = (slice(z_start, z_start+cshape),
                  slice(y_start, y_start+cshape), 
                  slice(x_start, x_start+cshape))
            self.tiles.append(dat[sh[0], sh[1], sh[2]].copy())
            if threshold or background_limit:
                self.tile_indices.append((z_start,y_start,x_start))
            if self.lab is not None:
                self.ltiles.append(lab[sh[0], sh[1], sh[2]].copy())
            if self.weight is not None:
                self.wtiles.append(weight[sh[0], sh[1], sh[2]].copy())
    ...
    def recompose(self, tiles=False):
        rec = self.pred if not tiles else self.tiles
        # only recomposing predictions, as data and lab are retained anyway after decompose, unless tiles=True
        if rec is None:
        ...
        dat = np.zeros(self.orig_shape, dtype=np.float32)   # need a float in case recomposing real map (not labels)
        t_id = 0
        for i in range(0, dat.shape[0], self.step):  # [:-1] # self.padded_shape[0]):
           for j in range(0, dat.shape[1], self.step):  # [:-1] # self.padded_shape[1]):
               for k in range(0, dat.shape[2], self.step):  # [:-1] # self.padded_shape[2]):
                    ...
                    # print('Pred shape: ', rec[t_id].shape)
                    # REFACTOR: this does not account for correct amount of margin/overlap in the central (non-edge)
                    # tiles to avoid the influence of tile borders at each pass of the network
                    # we only want to keep the area of the tile that is not affected by tile edges (tilex-2*pool^depth)
                    # pt = rec[t_id][max(self.margin-i,0):, max(self.margin-j,0):, max(0,self.margin-k):]
                    pt = rec[t_id][self.margin:-self.margin, self.margin:-self.margin, self.margin:-self.margin]
                    #if any(i != self.step for i in pt.shape): continue
                    # print('Pred shape to stitch: ', pt.shape)
                    # x = i + self.cshape - self.margin if i + self.cshape - self.margin <= dat.shape[0] else dat.shape[0]
                    # y = j + self.cshape - self.margin if j + self.cshape - self.margin <= dat.shape[1] else dat.shape[1]
                    # z = k + self.cshape - self.margin if k + self.cshape - self.margin <= dat.shape[2] else dat.shape[2]
                    ...
                    #print('Slice to copy: ',x,y,z)
                    # convert indices to match orig shape
                    #print('Indices:',i,j,k)
#                     i = max(0,i - self.margin)
#                     j = max(0,j - self.margin)
#                     k = max(0,k - self.margin)
                    # print('Indices orig: ',i,j,k)
                    try:
...
def amino_map(data, orig, cell, path, overwrite=False):
    ...
    :param path: (str) directory to save data at
    :param overwrite: (bool) if True overwriting exiting labels, if False and the map exists, runtime error is raised
    """
    if len(np.unique(data)) == 1:
        print("WARNING: map has no unique values, so maps for each label won't be saved.")
...
def data_crop(sh, d, orig=None, samp=None, cell=None, mid=None, move_o=False):
    """
...
def data_scale(d, sh, orig=None, samp=None, cell=None):
    ...
    :return: d (np.ndarray) new padded isotropic 3D data
             ...
             cell (np.recarray[float]) updated cell size, only returned if either of orig, sample and cell is passed
    """
    # TODO right now all the default params are not needed, in future all data_* functions will be interfaced
    if orig is not None and samp is not None and cell is not None:
...
def legacy_atomic_model(path):
    """Legacy simple model reader. Use 'load_model' powered by BioPython.PDB instead."""
    ...
    for i, label in enumerate(nodes):
        scalar, xs, ys, zs = nodes[label]
        scalars.append(atoms[scalar])
        x.append(float(xs))
        y.append(float(ys))
        z.append(float(zs))
    x = np.asarray(x)
    ...
    scalars = 1.5 * np.max(scalars) - scalars

    x = np.linspace(np.min(x), np.max(x), len(x))
    y = np.linspace(np.min(y), np.max(y), len(y))
    z = np.linspace(np.min(z), np.max(z), len(z))
    print(len(x))
    print(len(y))
    print(len(z))
    print(len(scalars))

    # a = np.zeros((len(x), len(y), len(z)), dtype=np.int8)

    s = np.array([scalars[i] for (i, xyz) in enumerate(zip(np.ravel(x), np.ravel(y), np.ravel(z)))])
    s = s.reshape(x.shape)
    print(s.shape)
