...
from src.viewer import plot_metric, confusion_matrix


def class_occurence(data, mode='penal_weight', labs=None, per_sample=False, appendix='',
                    verbose=False):
    modes = ['nums', 'props', 'plain_weight', 'penal_weight']

    if str(type(data)) == "<class 'tensorflow.python.framework.ops.EagerTensor'>":
        ys = data.numpy()
    elif isinstance(data, list) and str(type(data[0])) == "<class 'src.proteins.Sample'>":
    ...
    if per_sample:
        if verbose:
            print("\n-------------------------- CLASS PROPORTIONS PER CLASS {}:".format(appendix))
...
class PlotConfusion(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        confusion_matrix(self.y_val, y_pred_val.ravel(), self.labels, lab_names=self.lab_names,
        ...
        if self.x_test is not None and self.y_test is not None:
            ...
            confusion_matrix(self.y_test, y_pred_test, self.labels, lab_names=self.lab_names,
        ...
        if self.x_train is not None and self.y_train is not None:
            ...
            confusion_matrix(self.y_train, y_pred_train, self.labels, lab_names=self.lab_names,
                             save='epoch{:04d}_trn.png'.format(epoch+1),
                             title="Training Confusion Matrix | Epoch: {}".format(epoch+1), ts=self.ts)

        if self.sample is not None:
            print("\nWARNING: Saving validation predictions, sample {}.".format(self.sample.id))
...
def train_unet(data, lab_names=None, epochs=50, batch=1, arch_mode='large', loss_mode='scce',
               patience=30, querymaps=False, verbose=False, callbacks=None):
    # TODO paralelise this
    if not str(type(data[0])) == "<class 'src.proteins.Sample'>":
        raise RuntimeError("Data must be of type src.protein.Sample, not {}.".format(type(data[0])))
    ...
    class_occurence(d_train, mode='props', per_sample=querymaps, appendix='train', verbose=True)
    class_occurence(d_val, mode='props', per_sample=querymaps, appendix='val', verbose=True)
    class_occurence(d_test, mode='props', per_sample=querymaps, appendix='test', verbose=True)
    ...
    if 'checkpoints' in callbacks:
        ...
        cbs.append(tf.keras.callbacks.ModelCheckpoint(
            os.path.join('logs/checkpoints', mname + '_epoch{epoch:04d}_checkpoint.h5'), save_weights_only=False))
    if 'stopping' in callbacks:
        cbs.append(tf.keras.callbacks.EarlyStopping(monitor='loss',
                                                    patience=patience))
    ...
    if 'confusion' in callbacks:
        ...
        cbs.append(PlotConfusion(labels, x_val, y_val, x_test=tests[0], y_test=tests[1],
                                 x_train=trains[0], y_train=trains[1], maps=maps, ts=ts, lab_names=lab_names))
...
def predict_unet(data, model_name="model_64x64x64_final.h5", batch=1, evaluate=False, querymaps=False, verbose=False):
    if not os.path.exists(model_name):

        if len(model_name.split('/')) <= 1:

            print("\nWARNING: Model {} does not exist, searching available models in \"logs/checkpoints\" instead.".format(model_name))

            if not os.path.exists("../logs/checkpoints") or len(os.listdir("../logs/checkpoints")) == 0:
                raise RuntimeError("No trained models available.")

            if model_name not in os.listdir("../logs/checkpoints"):
                print("\nWARNING: Model {} does not exist in directory \"logs/checkpoints\". Chosing the last saved checkpoint instead".format(model_name))
    ...
    if evaluate:
        class_occurence(data, mode='props', per_sample=querymaps, appendix='pred_eval', verbose=True)
    ...
    # xdata = np.expand_dims([d.map for d in data], axis=-1)
        ...
        xdata = reshape_data(data, eval=False)[0]
