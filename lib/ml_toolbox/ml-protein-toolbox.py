...
from src.analysis import class_occurence, train_unet, predict_unet
from src.multi_proteins import load_data, preproces_data, query_data, plot_data, gen_labs, mpi_gen_labs
...
nv_help = 'Value for normalising voxel size.'
mn_help = 'Minimum voxel size allowed, if outside the bounds normalising to 1.0.'
mx_help = 'Maximum voxel size allowed, if outside the bounds normalising to 1.0.'
s_help = 'If -p/--preprocess set to \'crop\', use this to specify shape of the cropped image'
th_help = 'If using tiles with background limit, this option sets the threshold of background for the map.' \
          'Tiles with background more than th will be rejected.'
bg_help = 'If using tiles, this option sets percentage of background limit for accepted tiles.'
bl_help = 'If using tiles with background limit, this option thresholds tiles by label mask (if labels exist).'
...
ag_help = 'When -c/--classes is set to residue, this option will group the residues into 6 shape groups.'
...
@click.option('--nvox', '-nv', help=nv_help, default=0.7)
@click.option('--vmin', '-mn', help=mn_help, default=0.6)
@click.option('--vmax', '-mx', help=mx_help, default=1.0)
@click.option('--cshape', '-s', help=s_help, type=int, default=64)
@click.option('--background', '-bg', help=bg_help, type=float, default=None)
@click.option('--threshold', '-th', help=th_help, type=float, default=None)
@click.option('--bglab', '-bl', help=bl_help, type=int, default=None)
...
@click.option('--model', '-h', help=h_help, type=str, default="model_64x64x64_final.h5")
# genlabs opts
...
@click.option('--aminog', '-ag', help=ag_help, type=bool, default=False, is_flag=True)
def run(datapath, res, n, classes, preproces, nvox, vmin, vmax, cshape, background, threshold, bglab, normalise, querymaps,
        utility, epochs, batch, arch, loss, callback,
        evaluate, model, mpilabs, genmaps, two_sigma, json_labs, aminog):
    ...
    if utility == 'makelabels':
        ...
        if mpilabs:
            mpi_gen_labs(datapath, res=list(res), custom_labs=custom_labs, two_sigma=two_sigma,
    ...
    if datapath:
        ...
        data = load_data(datapath, res, n, predict=pred, weights=wght, amino_groups=aminog)

        if not pred:
            # 2) SET UP LABELS TODO there are better ways to do this
            if aminog and classes != 'residue':
                raise RuntimeError('Param aminog can only be used with residue -c/--classes.')
            if classes == 'residue':
                if aminog:
                    labels = ['0', 'G', 'P', 'L', 'K', 'Y', 'W', 'O']
                else:
                    labels = ['0', 'ALA', 'CYS', 'ASP', 'GLU', 'PHE', 'GLY', 'HIS', 'ILE', 'LYS', 'LEU', 'MET', 'ASN',
                              'PRO', 'GLN', 'ARG', 'SER', 'THR', 'VAL', 'TRP', 'TYR', 'o']
        ...
        # 3) PREPROCESS DATA
        data = preproces_data(data, mode=preproces, cshape=cshape,
                              threshold=threshold, background=background, background_label=bglab,
                              norm=normalise, norm_vox=nvox, norm_vox_lim=(vmin, vmax))
    else:
